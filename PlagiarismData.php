...
class PlagiarismData extends Database
{
    ...
    {
        list("userId" => $userId, "reportId" => $reportId,"type"=>$type) = $reportData;
        $query = "INSERT INTO `checks`(`user_id`,`report_id`,`type`) VALUES('$userId','$reportId','$type')";
        return $this->query($query);
    }

    function getAllUserChecks(int $userId, string $type = "NORMAL")
    {
        $query = "SELECT `checks`.* FROM `checks` INNER JOIN `users` ON `users`.`user_id` = `checks`.`user_id` WHERE `users`.`user_id` = '$userId' AND `checks`.`type` ='$type' ORDER BY `checks`.`created_at` DESC";
